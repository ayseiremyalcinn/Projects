...
public class Project {
    ...
    public int[] getEarliestSchedule() {
        int[] shedule = new int[tasks.size()];
        for (int i = 0; i < tasks.size(); i++) {
            shedule[i] = -1;
        }
        for (int i = 0; i < getProjectDuration(); i++) {
            for (int j = 0; j < tasks.size(); j++) {
                if(shedule[j] == -1){
                    boolean dont = false;
                    for (int k = 0; k < tasks.get(j).getDependencies().size(); k++) {
                        int startimeK =  shedule[tasks.get(j).getDependencies().get(k)];
                        if(startimeK == -1 || startimeK + tasks.get(tasks.get(j).getDependencies().get(k)).getDuration() > i){
                            dont = true;
                            break;
                        }
                    }
